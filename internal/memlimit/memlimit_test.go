...
func TestDivide(t *testing.T) {
	...
	t.Run("splits a 16GiB ceiling into fixed shares", func(t *testing.T) {
		plan, err := Limit{Ceiling: ceiling, Source: "test"}.Divide(16, 0)
	...
	t.Run("every instance together stays inside the budget", func(t *testing.T) {
		...
		for _, c := range []struct {
			...
			plan, err := Limit{Ceiling: c.ceiling}.Divide(c.dbs, c.preset)
			require.NoErrorf(t, err, "ceiling=%d preset=%d dbs=%d", c.ceiling, c.preset, c.dbs)
			require.LessOrEqualf(t, plan.DuckDBPerDB*int64(c.dbs)+plan.GoHeap, plan.Budget,
	...
	t.Run("an existing GOMEMLIMIT takes from DuckDB, not from the ceiling", func(t *testing.T) {
		...
		plan, err := Limit{Ceiling: 2 << 30}.Divide(16, preset)
		require.NoError(t, err)
		require.True(t, plan.GoHeapPreset)
		require.Equal(t, preset, plan.GoHeap)
		require.Equal(t, "67MB", plan.MemoryLimit())
	})

	t.Run("a GOMEMLIMIT that starves DuckDB is refused, not rounded up", func(t *testing.T) {
		// 87.5% of RAM leaves 25MB per database. Starting on that means dying
		// at the first query instead of at startup.
		_, err := Limit{Ceiling: 16 << 30}.Divide(16, 14<<30)
		require.ErrorIs(t, err, ErrTooSmall)
		require.ErrorContains(t, err, "64MiB is the minimum")
	})

	t.Run("a ceiling too small for the databases is refused", func(t *testing.T) {
		_, err := Limit{Ceiling: 512 << 20}.Divide(16, 0)
		require.ErrorIs(t, err, ErrTooSmall, "12MB per database cannot build an index")
	})

	t.Run("the Go share has a floor", func(t *testing.T) {
		plan, err := Limit{Ceiling: 8 << 30}.Divide(1, 0)
		require.NoError(t, err)
		require.GreaterOrEqual(t, plan.GoHeap, int64(minGoHeap))

		// 30% of this budget is under the floor, so the floor is what applies.
		small, err := Limit{Ceiling: 900 << 20}.Divide(1, 0)
		require.NoError(t, err)
	...
	t.Run("the divisor is what shrinks the per-database share", func(t *testing.T) {
		one, err := Limit{Ceiling: ceiling}.Divide(1, 0)
		require.NoError(t, err)
		many, err := Limit{Ceiling: ceiling}.Divide(16, 0)
		require.NoError(t, err)
